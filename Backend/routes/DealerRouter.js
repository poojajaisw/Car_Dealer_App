...
router.post("/signup", async (req, res) => {
  try {
    ...
    if (!dealership_email || !password || !dealership_name || !dealership_location || !dealership_info) {
...
// router for dealing car from dealer
...
router.post("/dealerlogin", async (req, res) => {
  try {
    ...
    if (didMatch) {
      const token = jwt.sign({ _id: dealershipInDB._id }, JWT_SECRET, { expiresIn: process.env.JWT_EXPIRY || '1h' });
      res.json({ token });
    } else {
